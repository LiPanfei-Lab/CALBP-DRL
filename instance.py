...
class Instance(object):
    ...
    def get_dependency(self):
        ...
        for pred, succ in self.immediate_precedence:
            self.dependencies[succ].append(pred)
    ...
    def get_process_level(self):
        ...
        def calculate_level(node):
            ...
            preds = self.dependencies.get(node, [])
            if not preds:
                node_levels[node] = 0
                return 0

            level = max(calculate_level(pred) for pred in preds) + 1
            node_levels[node] = level
            return level

        for process in self.processes:
            calculate_level(process)
        ...
        self.process_levels = node_levels
    ...
    def get_workers_capable_processes(self):
        ...
        # todo
        self.pros_have_capable_category_workers = {k for k, v in self.pros_category_capable_workers.items() if v}
        self.pros_common_capable_workers = {k: self.pros_skill_capable_workers[k].union(self.pros_category_capable_workers[k])
                                            for k in self.pros_have_capable_skill_workers
                                            if k in self.pros_have_capable_category_workers}

        # print(self.processes.keys())
        # print(self.pros_have_capable_skill_workers)
        # print(self.pros_have_capable_category_workers)
        # print(self.pros_common_capable_workers.keys())

    def is_w_p_valid(self, w, p):
        if w in self.pros_skill_capable_workers[p]:
            return True
        if w in self.pros_category_capable_workers[p]:
            return True
        return False
