...
class Solution(Instance):
    ...
    def valid_worker_capability(self):
        ...
        for p, w in product(self.processes, self.workers):
            if p in self.pros_common_capable_workers:
                if w not in self.pros_common_capable_workers[p]:
                    assert self.worker_to_process[w, p] == 0, \
                        (f"Process [{p}] is assigned to a category capable worker [{w}], "
                         f"when there is other skill capable worker.")
                if self.worker_to_process[w, p] == 1:
                    if w not in self.pros_common_capable_workers[p]:
                        print("Error!")
                    # assert w in self.pros_common_capable_workers[p] \
                    #     (f"Process [{p}] is assigned to a category capable worker [{w}], "
                    #      f"when there is other skill capable worker.")
            elif p in self.pros_have_capable_skill_workers:
                if w not in self.pros_skill_capable_workers[p]:
                    assert self.worker_to_process[w, p] == 0, \
                        (f"Process [{p}] is assigned to a category capable worker [{w}], "
                         f"when there is other skill capable worker.")
                if self.worker_to_process[w, p] == 1:
                    ...
                    #     (f"Process [{p}] is assigned to a category capable worker [{w}], "
                    #      f"when there is other skill capable worker.")
            elif p in self.pros_have_capable_category_workers:
                if w not in self.pros_category_capable_workers[p]:
                    assert self.worker_to_process[w, p] == 0, \
                        (f"Process [{p}] is assigned to a category capable worker [{w}], "
                         f"when there is other skill capable worker.")
                if self.worker_to_process[w, p] == 1:
                    if w not in self.pros_category_capable_workers[p]:
                        print("Error!")
                    # assert w in self.pros_category_capable_workers[p] \
                    #     (f"Process [{p}] is assigned to a category capable worker [{w}], "
                    #      f"when there is other skill capable worker.")
            else:
                print(f"Warning: Process [{p}] is not considered in decision variable.")
        return True
